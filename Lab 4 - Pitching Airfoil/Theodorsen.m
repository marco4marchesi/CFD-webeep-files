...
a = -1;             % distance from half chord to rotation center adimensionalized by b
...
%% Theodorsen correction to account for finite thickness
%
...
plot(alpha,CL,'-r',alpha,CL_mod,'-b','linew',1.3);

%% numerical data - RANS
file = "../RANS/CFD_100/history.csv";

T = readtable(file);

dt = 9.231914044466431e-04*1;  % time step used in numerical simulation (100 time steps per period)
% dt = 9.231914044466431e-04*2;  % time step used in numerical simulation (50 time steps per period)
% dt = 9.231914044466431e-04*4;  % time step used in numerical simulation (25 time steps per period)
...
tspp = ceil((2*pi/omega)/dt);                           % time steps per period
plot(aa(end-tspp:end),T.CL(end-tspp:end),'-o','linew',1); % plot last period

%% numerical data - EULER
...
hold on; box on; grid on;
tspp = ceil((2*pi/omega)/dt);                           % time steps per period
plot(aa(end-tspp:end),T.CL(end-tspp:end),'-x','linew',1); % plot last period


% labels
tz=legend('Theodorsen', 'Motta et al.', 'SU2 RANS', 'SU2 EULER');
ty=ylabel('$C_{\ell}$');
tx=xlabel('$\alpha$');
