...
SU2 = readtable('../HYBRID/coarse.csv');
x = SU2.Points_0;
y = -SU2.Pressure_Coefficient;
order = reorderPoints(x,y);
plot(x(order),y(order),'linew',1.5);

SU2 = readtable('../HYBRID/medium.csv');
x = SU2.Points_0;
y = -SU2.Pressure_Coefficient;
order = reorderPoints(x,y);
plot(x(order),y(order),'linew',1.5);

SU2 = readtable('../HYBRID/fine.csv');
x = SU2.Points_0;
y = -SU2.Pressure_Coefficient;
order = reorderPoints(x,y);
plot(x(order),y(order),'linew',1.5);

% SU2 = readtable('../CGRID/cgrid.csv');
% x = SU2.Points_0;
% y = -SU2.Pressure_Coefficient;
% order = reorderPoints(x,y);
% plot(x(order),y(order),'linew',1.5);
...
tx = xlabel('$x$');
ty = ylabel('$C_P$');
tz = legend('DATA','coarse','medium','fine');
